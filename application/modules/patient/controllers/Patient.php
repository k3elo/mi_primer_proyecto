...
class Patient extends MX_Controller {
    ...
    public function addNew() {
            ...
            // 1. Verificar permisos: Si el usuario es un "Paciente", redirigirlo a la página de "permission"
        if ($this->ion_auth->in_group(array('Patient'))) {
            redirect('home/permission');
        }
        error_reporting(E_ALL);
        ini_set('display_errors', 1);
        // 2. Recuperar datos del formulario (POST y GET):
        $id = $this->input->post('id'); // ID del paciente (si se está editando)
        $redirect = $this->input->get('redirect'); // URL de redirección (si se especifica)
        if (empty($redirect)) {
            $redirect = $this->input->post('redirect');
        }
        $rut = $this->input->post('rut'); // RUT del paciente
        $name = $this->input->post('name'); // Nombre del paciente
        $password = $this->input->post('password'); // Contraseña del paciente
        $sms = $this->input->post('sms'); // No se utiliza en el código proporcionado
        $doctor = $this->input->post('doctor'); // ID del doctor asignado
        $address = $this->input->post('address'); // Dirección del paciente
        $phone = $this->input->post('phone'); // Teléfono del paciente
        $sex = $this->input->post('sex'); // Sexo del paciente
        $birthdate = $this->input->post('birthdate'); // Fecha de nacimiento del paciente
        $bloodgroup = $this->input->post('bloodgroup'); // Grupo sanguíneo del paciente
        $patient_id = $this->input->post('p_id'); // ID de registro del paciente

        // 3. Preparar datos para la base de datos:
        if (empty($id)) { // Si no se proporciona ID, es un nuevo paciente
            $add_date = date('m/d/y'); // Fecha de registro
            $registration_time = time(); // Hora de registro
        } else { // Si se proporciona ID, es una edición
            $add_date = $this->patient_model->getPatientById($id)->add_date; // Obtener fecha de registro existente
            $registration_time = $this->patient_model->getPatientById($id)->registration_time; // Obtener hora de registro existente
        }

        if (empty($patient_id)) { // Generar ID de registro si no se proporciona
            $patient_id = rand(10000, 10000000);
        }

        $email = $this->input->post('email'); // Correo electrónico del paciente
        if (empty($email)) { // Generar correo electrónico si no se proporciona
        ...
        if (empty($id)) { // La contraseña es requerida solo para nuevos pacientes
        ...
        if ($this->form_validation->run() == FALSE) { // Si la validación falla
        ...
        } else { // Si la validación es exitosa

            // 6. Subir imagen (si se proporciona):
            ...
            if ($this->upload->do_upload('img_url')) { // Si la subida es exitosa
                $path = $this->upload->data();
                $img_url = "uploads/" . $path['file_name'];
                $data = array( // Datos para la base de datos (con imagen)
                ...
                $data = array( // Datos para la base de datos (sin imagen)
                    'patient_id' => $patient_id,
                    'id' => $rut,
                    'name' => $name,
                    'email' => $email,
            ...
            // 7. Insertar o actualizar datos en la base de datos:
            $username = $this->input->post('name');

        if (empty($id)) { // Si es un nuevo paciente

                $dfg = 5; //  ID del grupo.  Debería obtenerse dinámicamente, no hardcoded.
                $this->ion_auth->register($username, $password, $email, $dfg); // Registrar al usuario en la tabla 'users' (Ion Auth)
                $ion_user_id = $this->db->get_where('users', array('email' => $email))->row()->id; // Obtener el ID del usuario recién registrado
                $this->patient_model->insertPatient($data); // Insertar los datos del paciente en la tabla 'patient'
                $patient_user_id = $this->db->get_where('patient', array('email' => $email))->row()->id; // Obtener el ID del paciente recién insertado
                $id_info = array('ion_user_id' => $ion_user_id); // Crear un array con el ID del usuario (Ion Auth)
                $this->patient_model->updatePatient($patient_user_id, $id_info); // Actualizar la tabla 'patient' con el 'ion_user_id'

                // --- Inicio de envío de SMS ---
                $set['settings'] = $this->settings_model->getSettings(); // Obtener la configuración del sistema
                $autosms = $this->sms_model->getAutoSmsByType('patient'); // Obtener la configuración de SMS automático para pacientes
                $message = $autosms->message; // Obtener el mensaje SMS
                $doctorname = $this->doctor_model->getDoctorById($doctor)->name; // Obtener el nombre del doctor
                $to = $phone; // Número de teléfono del paciente
                $name1 = explode(' ', $name); // Separar el nombre en nombre y apellido
                if (!isset($name1[1])) {
                    $name1[1] = null; // Si no hay apellido, asignar null
                }
                $data1 = array( // Datos para el mensaje SMS
                    'firstname' => $name1[0],
                    'lastname' => $name1[1],
                    'name' => $name,
                    'doctor' => $doctorname,
                    'company' => $set['settings']->system_vendor // Nombre de la empresa
                );

                if ($autosms->status == 'Active') { // Si el envío de SMS automático está activo
                    $messageprint = $this->parser->parse_string($message, $data1); // Parsear el mensaje con los datos
                    $data2[] = array($to => $messageprint); // Crear un array con el número y el mensaje
                    $this->sms->sendSms($to, $message, $data2); // Enviar el SMS
                }
                // --- Fin de envío de SMS ---

                // --- Inicio de envío de correo electrónico ---
                $autoemail = $this->email_model->getAutoEmailByType('patient'); // Obtener la configuración de correo electrónico automático para pacientes
                if ($autoemail->status == 'Active') { // Si el envío de correo electrónico automático está activo
                    $mail_provider = $this->settings_model->getSettings()->emailtype; // Obtener el tipo de proveedor de correo electrónico
                    $settngs_name = $this->settings_model->getSettings()->system_vendor; // Nombre del sistema
                    $email_Settings = $this->email_model->getEmailSettingsByType($mail_provider); // Obtener la configuración del proveedor de correo electrónico
                    $message1 = $autoemail->message; // Obtener el mensaje de correo electrónico
                    $messageprint1 = $this->parser->parse_string($message1, $data1); // Parsear el mensaje con los datos

                    if ($mail_provider == 'Domain Email') { // Configurar el remitente del correo electrónico según el proveedor
                    ...
                    $this->email->to($email); // Destinatario del correo electrónico
                    $this->email->subject('Registration confirmation'); // Asunto del correo electrónico
                    $this->email->message($messageprint1); // Cuerpo del correo electrónico
                    $this->email->send(); // Enviar el correo electrónico


                    // --- Envío de correo electrónico con detalles de la cuenta ---
                    $mail_provider = $this->settings_model->getSettings()->emailtype; // Obtener el tipo de proveedor de correo electrónico
                    $settngs_name = $this->settings_model->getSettings()->system_vendor; // Nombre del sistema
                    $emailSettings = $this->email_model->getEmailSettingsByType($mail_provider); // Obtener la configuración del proveedor de correo electrónico
                    $base_url = str_replace(array('http://', 'https://', '/'), '', base_url()); // Obtener la URL base del sitio
                    $subject = $base_url . ' - Patient Registration Details'; // Asunto del correo electrónico
                    $message = 'Dear ' . $name . ', Thank you for the registration. <br> Here is your login details.<br> <br> Link: ' . base_url() . 'auth/login <br> Username: ' . $email . ' <br> Password: ' . $password . '<br><br> Thank You, <br>' . $this->settings->title; // Cuerpo del correo electrónico con detalles de la cuenta
                    ...
                    $this->email->to($email);  // Destinatario del correo electrónico
                    $this->email->subject($subject); // Asunto del correo electrónico
                    $this->email->message($message); // Cuerpo del correo electrónico
                    $this->email->send(); // Enviar el correo electrónico
                }
            ...
            } else { // Si se proporcionó una nueva contraseña
                $password = $this->ion_auth_model->hash_password($password); // Encriptar la nueva contraseña
            }
        ...
        // como guardar o actualizar datos de un paciente.

        if (!empty($redirect)) { // Verifica si la variable $redirect no está vacía.
            // $redirect probablemente contiene una URL a la que se desea redirigir al usuario.
            redirect($redirect); // Redirige al usuario a la URL almacenada en $redirect.
        } else { // Si $redirect está vacía (no se ha especificado una URL de redirección).
            redirect('patient'); // Redirige al usuario a la URL 'patient'.
            // Esta URL probablemente corresponde a la página principal de la sección de pacientes.
        }

        // En resumen:
        // Si se proporciona una URL en $redirect, el usuario es redirigido a esa URL.
        // Si no se proporciona ninguna URL, el usuario es redirigido a la página principal de pacientes.
        }
    }
    ...
    /* public function check_rut() {
        ...
        // Sanitización del RUT (MUY IMPORTANTE)
        $rut = preg_replace('/[^0-9kK]/', '', $rut); // Elimina caracteres no válidos
    
        // Consulta a la base de datos (usando active record)
        $this->db->where('id', $rut);
        $query = $this->db->get('patient'); // Reemplaza 'patients' con el nombre de tu tabla
        ...
        echo json_encode($data); // Devuelve la respuesta en formato JSON
    } */
    function editPatient() { // Función para editar un paciente
        $data = array(); // Se crea un array vacío llamado $data. Este array se utilizará para pasar datos a las vistas.    
        $id = $this->input->get('id'); // Obtiene el valor del parámetro 'id' que se haya pasado a través de la URL (usando el método GET). Este 'id' se supone que es el ID del paciente que se va a editar. Por ejemplo, la URL podría ser algo como tu_controlador/editPatient?id=123.    
        $data['patient'] = $this->patient_model->getPatientById($id); // Llama al método getPatientById($id) del modelo patient_model. Este método debería consultar la base de datos y devolver los datos del paciente con el ID especificado. Los datos del paciente se almacenan en el array $data bajo la clave 'patient'.    
        $data['doctors'] = $this->doctor_model->getDoctor(); // Llama al método getDoctor() del modelo doctor_model. Este método debería consultar la base de datos y devolver una lista de todos los doctores. Esta lista se almacena en el array $data bajo la clave 'doctors'. Probablemente se usará para un dropdown o select en el formulario de edición.    
        $data['groups'] = $this->donor_model->getBloodBank(); // Llama al método getBloodBank() del modelo donor_model. Este método debería consultar la base de datos y devolver una lista de los grupos sanguíneos disponibles. Esta lista se almacena en el array $data bajo la clave 'groups'. Probablemente se usará también para un dropdown o select.
        $this->load->view('home/dashboard'); // Carga la vista 'dashboard' que se encuentra dentro del directorio 'home' de tus vistas. Esta vista probablemente contiene la estructura principal de tu página (cabecera, menú, etc.).
        $this->load->view('add_new', $data); // Carga la vista 'add_new'. Esta vista probablemente contiene el formulario para editar los datos del paciente. El segundo parámetro, $data, pasa los datos (paciente, doctores, grupos sanguíneos) a la vista 'add_new' para que puedan ser mostrados en el formulario.   
        $this->load->view('home/footer'); // Carga la vista 'footer' que se encuentra dentro del directorio 'home'. Esta vista probablemente contiene el pie de página de tu sitio web.    
    }
